...
test_that("lanczos vectors require k>1", {
  c1 = coo(matrix(1:9, ncol=3))
  expect_error(lanczos.coo(c1, 1))
})


test_that("lanczos vectors for a connectivity matrix", {
  ## construct a medium sized graph matrix
  mat = matrix(0, ncol=12, nrow=12)
  for (i in 1:ncol(mat)) {
    ip2 = 1+ (c(i, i+1) %% nrow(mat))
    mat[i, ip2] = abs(rnorm(2))
  }
  diag(mat) = 1
  mat = mat + t(mat)

  ## compute eigenvectors with coos
  result = spectral.coo(reduce.coo(coo(mat)), 3)[,1:2]
